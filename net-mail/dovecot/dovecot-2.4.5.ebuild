...
EAPI=7
inherit autotools ssl-cert systemd toolchain-funcs flag-o-matic user

DESCRIPTION="An IMAP and POP3 server written with security primarily in mind"
HOMEPAGE="https://www.dovecot.org/"
SRC_URI="
https://www.dovecot.org/releases/2.4/dovecot-2.4.5.tar.gz -> dovecot-2.4.5.tar.gz
sieve? ( https://pigeonhole.dovecot.org/releases/2.4/dovecot-pigeonhole-2.4.5.tar.gz -> dovecot-pigeonhole-2.4.5.tar.gz )"
...
src_configure() {
	...
	if use sieve ; then
		...
		cd "../dovecot-pigeonhole-2.4.5" || die "cd failed"
		econf \
			$( use_enable static-libs static ) \
			--localstatedir="/var" \
			--enable-shared \
...
src_compile() {
	default
	if use sieve ; then
		pushd "../dovecot-pigeonhole-2.4.5" > /dev/null || die
